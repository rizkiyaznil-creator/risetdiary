...
- **Stack:** Next.js 16 (App Router, TS) · Prisma 7 + SQLite (→ Postgres saat deploy) · auth email+password native (jose + bcryptjs) · Tailwind v4.
- **Catatan teknis (penting, beda dari dok lama):**
  ...
  - Prisma 7: tanpa engine bawaan → pakai **driver adapter** (`better-sqlite3`); client di-`generate` ke `app/generated/prisma`; `migrate dev` tidak auto-generate; **SQLite tak dukung enum** (status/peran = teks).
